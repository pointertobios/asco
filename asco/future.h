...
public:
    ...
    public:
        ...
        auto final_suspend() noexcept {
            ...
            auto hdl = core::worker::current().pop_handle();
            assert(hdl == this->m_future_object->m_this_handle);
            return final_awaitable{
                this->m_future_object->m_this_handle, this->m_future_object->m_caller_handle};
        }
    };
    ...
    void bind_lambda(std::unique_ptr<util::erased> l) noexcept { m_bound_lambda = std::move(l); }
...
private:
    ...
    std::unique_ptr<util::erased> m_bound_lambda;
};
