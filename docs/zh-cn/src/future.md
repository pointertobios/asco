...
* 当前协程与 lambda 表达式在同一工作线程中被调度执行：当前协程执行至 `co_await t` 挂起后，线程栈退出当前栈帧，如果 `flag` 被优化到栈上，
  会失效， lambda 表达式中捕获的 `flag` 将引用一个已经退出的栈帧中的地址。
...
> 本框架**不限制**其它错误处理方式的使用。
>
> 本框架不以异常作为错误处理的方式，异常仅用于调试代码。若本框架内部抛出异常，可能是本项目的 bug 或使用方式不对产生的。
...
在 `co_return` 后，析构阶段无法抛出异常，但是可以通过 `this_coro::throw_coroutine_abort()` 直接让协程抛出异常。
