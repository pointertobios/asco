...
asco::future<void> test_constructors() {
    ...
    asco::println("Constructors tested successfully.");
    co_return;
}
...
future<void> test_locks() {
...

    {
        ...
        asco::println("Read lock acquired: {}", *read_guard);
    }

    {
        auto write_guard = co_await lock.write();
        *write_guard = 20;
        asco::println("Write lock acquired and value updated: {}", *write_guard);
    }
...
future<void> test_nested_locks() {
...

    {
        ...
        asco::println("Nested read locks acquired: {}, {}", *read_guard1, *read_guard2);
    }

    {
        auto write_guard1 = co_await lock.write();
        auto write_guard2 = lock.try_write();
        asco::print("Nested write locks acquired: {}", *write_guard1);
        if (write_guard2) {
            asco::println(", {}", **write_guard2);
        } else {
            asco::println(", {}", "none");
        }
    }
...
future<void> test_mutex_locks() {
    ...
    auto task = co_await lock.write().then([](rwlock<int>::write_guard write_guard) -> future<future<void>> {
        asco::println("Write lock acquired: {}", *write_guard);

        auto task = []() -> future<void> {
            rwlock<int> coro_local(lock);
            auto read_guard = co_await lock.read();
            asco::println("Read lock acquired after write lock released: {}", *read_guard);
            assert(*read_guard == 10);
            co_return;
        }();
        co_await asco::this_coro::sleep_for(1s);
        asco::println("Write lock released");
        co_return task;
    });
...
future<void> test_exception_handling() {
    ...
    try {
        ...
        asco::println("Exception caught: {}", msg);
    }

    {
        auto read_guard = co_await lock.read();
        asco::println("Read lock acquired after exception: {}", *read_guard);
    }
